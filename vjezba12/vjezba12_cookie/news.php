<?php 
	...
	if (!isset($action)) {
		...
		if ($action == 1) {
			...
			setcookie('news_title_1', 'US growth faster than expected', time() + (60 * 60 * 24 * 7), '/');
			...
			# inicijalizacija kolačića
			setcookie('news_title_2', 'Wall Street: Tech firm surge pushes US markets higher', time() + (60 * 60 * 24 * 7), '/');
			...
			# inicijalizacija kolačića
			setcookie('news_title_3', 'Hotel booking sites probed by consumer watchdog', time() + (60 * 60 * 24 * 7), '/');
